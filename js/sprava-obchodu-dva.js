document.addEventListener('DOMContentLoaded', () => {
    ...
    const addItemBtn = document.getElementById('add-item-btn'); // Přidat položku z knihovny
    ...
    const loadShopItems = async (shopId) => {
        ...
        itemsRef.onSnapshot(async (snapshot) => {
            ...
            for (const doc of snapshot.docs) {
                ...
                const masterItemDoc = await db.collection('predmety_master').doc(itemData.masterItemId).get();
                
                if (masterItemDoc.exists) {
                    const masterItem = masterItemDoc.data();
                    html += `
                        <div class="table-row">
                            <div>${masterItem.nazev}</div>
    ...
    const openMasterItemSelectionModal = async () => {
        ...
        const snapshot = await db.collection('predmety_master').orderBy('nazev').get();
        if(snapshot.empty) {
    ...
    document.getElementById('master-item-selection-container').addEventListener('click', async (e) => {
        if (e.target.classList.contains('add-to-shop-btn')) {
            ...
            if (price !== null && !isNaN(price) && price.trim() !== '') {
                ...
                // Není potřeba manuální reload, onSnapshot to zařídí
            } else if (price !== null) {
    ...
    // Akce v seznamu obchodů (správa, mazání)
